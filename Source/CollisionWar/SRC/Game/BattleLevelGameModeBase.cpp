...
void ABattleLevelGameModeBase::UpdateCharacterNormalLogic(float dT)
{
	for (int32 i=0; i<m_pAllCharacters.Num(); i++)
	{
		...
		if ((m_pAllCharacters[i]->m_actionName == TEXT("ImplementSkill") || 
			...
			m_pAllCharacters[i]->m_actionName == TEXT("ImplementCounterStrike") ||
			m_pAllCharacters[i]->m_actionName == TEXT("ImplementAbsort")) && canMove == true)
		{
			if (m_pAllCharacters[i]->m_actionName == TEXT("ImplementSpellSkill"))
			{
				AKing* pKing = Cast<AKing>(m_pAllCharacters[i]);
				bool canBuild = CheckResourceAmoutToBuild(pKing->m_collectPhysOperationNb);
				if (!canBuild) continue;
				SubtractFCConstructResources(pKing->m_collectPhysOperationNb);
				SpawnConstructionSite(pKing->m_collectPhysOperationNb);
			}
			else if (m_pAllCharacters[i]->m_actionName == TEXT("ImplementBlock"))
			{
				m_pAllCharacters[i]->m_implementSkillType = 1;
				m_pAllCharacters[i]->ImplementSkill_Int(m_pAllCharacters[i]->m_campFlag);
			}
			else if (m_pAllCharacters[i]->m_actionName == TEXT("ImplementCounterStrike"))
			{
				m_pAllCharacters[i]->StopContinuousTask();
				m_pAllCharacters[i]->m_implementSkillType = 2;
				m_pAllCharacters[i]->ImplementSkill_Int(m_pAllCharacters[i]->m_campFlag);
			}
			else if (m_pAllCharacters[i]->m_actionName == TEXT("ImplementAbsort"))
			{
		...
		if (m_pAllCharacters[i]->m_characterType == 1)
		{
			//RecoverPlayerPerInterval();
			AKing* pPlayerCharacter = Cast<AKing>(m_pAllCharacters[i]);
			/*if (pPlayerCharacter->m_logicVelOperation.X == 0 && pPlayerCharacter->m_logicVelOperation.Y == 0)
				pPlayerCharacter->Brake_Int();
			else
			{
				pPlayerCharacter->CalPlayerCharacterVel_Int(canMove);
			}*/
			/*if (pPlayerCharacter->m_Hp > 0)
				pPlayerCharacter->MoveToTarget_Int(true, canMove);*/
			//收集所有待生成的FoundationCharacter名单，统一生成
			if (pPlayerCharacter->m_waitToSpawnFoundationCharacter)
			{
				APlayerController* pPlayerController = UGameplayStatics::GetPlayerController(GetWorld(), 0);
				ABattleLevelPlayerController* pBattlePlayerController = Cast<ABattleLevelPlayerController>(pPlayerController);
				
				FVector spawnLoc(pBattlePlayerController->m_targetBuildLogicLoc.X, pBattlePlayerController->m_targetBuildLogicLoc.Y, m_defaultSpawnHeight);
				FRotator spawnRot(0, 0, 0);
				AFoundationCharacter* pFoundationCharacter = GetWorld()->SpawnActor<AFoundationCharacter>(pPlayerCharacter->m_waitToSpawnFoundationCharacter, spawnLoc, spawnRot);
				int32 id = 1;
				InitialNewCharacter(pFoundationCharacter, 0, id, pBattlePlayerController->m_targetBuildLogicLoc, false);
				pPlayerCharacter->m_waitToSpawnFoundationCharacter = NULL;
			}
		}
		/*else if (m_pAllCharacters[i]->m_characterType == 2)
		{
			m_pAllCharacters[i]->m_curLogicForwardDir = m_pAllCharacters[i]->m_logicForwardOperation;
		}*/
		/*else
		{
			m_pAllCharacters[i]->MoveToTarget_Int(false, canMove);
		}*/
		//m_pAllCharacters[i]->UpdateCurSkillState(dT);
		//m_pAllCharacters[i]->UpdateAllSkills(dT);
...
void ABattleLevelGameModeBase::RemoveCharacterFromGame(ABaseCharacter* pCharacter)
{
	...
	{
		AFoundationCharacter* pFoundationCharacter = Cast<AFoundationCharacter>(pCharacter);
		m_pFoundationCharacters.Remove(pFoundationCharacter);
		RemoveIntrinsicEffectInfos(pFoundationCharacter);
	}
	else
	{
		AKing* pKing = Cast<AKing>(pCharacter);
...
ABaseCharacter* ABattleLevelGameModeBase::FindFoundationCharacterEntity(int32 entityID)
{
	for (int32 i = 0; i < m_pFoundationCharacters.Num(); i++)
	{
		if (m_pFoundationCharacters[i]->m_entityID == entityID)
			return m_pFoundationCharacters[i];
	}
	return NULL;
}
...
void ABattleLevelGameModeBase::DeleteTutorialStep()
{
	...
	if (m_tutorialSteps[0].isSpawnIndicator)
	{
		...
		else if (spawnIndicatorActorType == "FC")
		{
			for (int32 i = 0; i < m_pFoundationCharacters.Num(); i++)
			{
				if (!m_pFoundationCharacters[i] || m_pFoundationCharacters[i]->m_characterClassName != spawnIndicatorActorName) continue;
				pTargetActor = m_pFoundationCharacters[i];
				break;
			}
		}
...
void ABattleLevelGameModeBase::CollectIntrinsicEffectInfos(AFoundationCharacter* pCharacter)
{

}

void ABattleLevelGameModeBase::RemoveIntrinsicEffectInfos(AFoundationCharacter* pCharacter)
{
...
bool ABattleLevelGameModeBase::CheckResourceAmoutToBuild(int32 spellNb)
{
	return false;
}

void ABattleLevelGameModeBase::SubtractFCConstructResources(int32 FCNb)
{

}

void ABattleLevelGameModeBase::SpawnConstructionSite(int32 FCNb)
{
