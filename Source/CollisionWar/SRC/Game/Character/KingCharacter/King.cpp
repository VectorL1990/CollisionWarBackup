...
void AKing::ImplementCounterStrikeSkill()
{
	//生成反击几何体
	if (!m_pCSSkill || !m_pCSAttachPS.particleSystem) return;
	TArray<ABasePhysGeo*> spawnGeos;
	int32 geoNb = 0;
	float yaw = GetActorRotation().Yaw;
	for (int32 i = 0; i < m_pCSSkill->m_geoClass.Num(); i++)
	{
		if (!m_pCSSkill->m_geoClass[i]) continue;
		TSubclassOf<class UObject> geoClass = m_pCSSkill->m_geoClass[i];
		for (int32 j = 0; j < m_pCSSkill->m_spawnGeoNb[i]; j++)
		{
			int32 relativeLauchAngle = m_pCSSkill->m_spreadAngle[geoNb];
			...
			//FLogicVec2D relativeLauchLoc = UCollisionWarBpFunctionLibrary::GetPtRotateAroundPtByAlpha_Int(m_logicRelativeSpawnPoint, FLogicVec2D(0, 0), relativeLauchAngle + yaw)*m_pCSSkill->m_spawnDistanceAmplifiedList[geoNb];
			...
			ABasePhysGeo* pGeo = GetWorld()->SpawnActor<ABasePhysGeo>(m_pCSSkill->m_geoClass[i], launchLoc, launchRotator);
			...
			if (m_pCSSkill->m_isHostSkill)
				m_pControlPhysGeos.Add(pGeo);
			geoNb += 1;
		}
	}
	/*for (int32 i = 0; i < m_pCSSkill->m_appendHostEffectParams.Num(); i++)
	{
		ImplementSkillAttachEffect(m_pCSSkill->m_appendHostEffectParams[i], NULL);
	}*/
}

void AKing::ImplementAbsortSkill()
{
...
void AKing::MessAllSkillCard()
{
	...
	AddSkill(pCWGI->m_curPlayerSPInfo.selectSkillInfo.counterSkillName, -1, true, 1);
...
void AKing::UpdateAllSkills(float dT)
{
	...
	if (m_implementSkillType == 0)
	{
		...
		for (int32 i = 0; i < m_pBaseEquipment->m_pSkills.Num(); i++)
		{
			...
			if (m_pBaseEquipment->m_pSkills[i]->m_skillMotionTypeString == "PMS_TransientPunch")
			{
				...
				if (!m_pBaseEquipment->m_pSkills[i]->m_canImplementFlag)
				{
					...
					/*else
					{
						if (m_pBaseEquipment->m_pSkills[i]->m_freezeLogicTickTime > 0)
							CDPercent = (float)m_pBaseEquipment->m_pSkills[i]->m_skillFrameCount / (float)m_pBaseEquipment->m_pSkills[i]->m_freezeLogicTickTime;
						else CDPercent = 1.f;
					}*/
				}
				//else CDPercent = 1.f;
		...
		{
			//即使当前手持物体非此装备，仍应该对装备信息进行更新
			if (!m_pEquipmentInfos[i]) continue;
			for (int32 j = 0; j < m_pEquipmentInfos[i]->m_pSkills.Num(); j++)
			{
				if (!m_pEquipmentInfos[i]->m_pSkills[j]) continue;
				if (!m_pEquipmentInfos[i]->m_pSkills[j]->m_canImplementFlag)
				{
					//每个技能的计时+1帧
					//如该技能的当前计时大于等于CD值，解锁该技能
					m_pEquipmentInfos[i]->m_pSkills[j]->m_skillFrameCount += increaseTick;
					if (m_curSkillNb == j && m_pEquipmentInfos[i]->m_pSkills[j]->m_skillFrameCount >= m_pEquipmentInfos[i]->m_pSkills[j]->m_freezeMotionTime)
					{
						m_pEquipmentInfos[i]->m_pSkills[j]->m_skillFrameCount = 0;
						m_pEquipmentInfos[i]->m_pSkills[j]->m_canImplementFlag = true;
						m_pEquipmentInfos[i]->m_pSkills[j]->m_hasSpawnGeo = false;
						StopContinuousTask();
						if (m_curEquipNb == -1) isCurEquipDelete = true;
					}
				}
				if (isCurEquipDelete) break;
			}
			if (isCurEquipDelete) break;
		}*/
	}
	...
	{
		if (!m_pBlockSkill) return;
		if (!m_pBlockSkill->m_canImplementFlag)
		{
			//m_pBlockSkill->m_skillFrameCount += increaseTick;
			if (m_pBlockSkill->m_skillFrameCount == -1 && 
				(m_pBaseAnimInstance->m_motionStateString == "PMS_Block" || m_pBaseAnimInstance->m_motionStateString == "PMS_CanCounterStrike"))
			{
				StopContinuousTask();
				m_pBlockSkill->m_skillFrameCount = 0;
				m_pBlockSkill->m_canImplementFlag = true;
	...
	{
		if (!m_pAbsortSkill) return;
		if (!m_pAbsortSkill->m_canImplementFlag)
		{
			m_pAbsortSkill->m_skillFrameCount += increaseTick;
			if (m_pAbsortSkill->m_skillFrameCount >= m_pAbsortSkill->m_freezeMotionTime)
			{
				StopContinuousTask();
				m_pAbsortSkill->m_skillFrameCount = 0;
				m_pAbsortSkill->m_canImplementFlag = true;
				m_pAbsortSkill->m_hasSpawnGeo = false;
			}
	...
	{
		if (!m_pCounterStrikeSkill) return;
		if (!m_pCounterStrikeSkill->m_canImplementFlag)
		{
			m_pCounterStrikeSkill->m_skillFrameCount += increaseTick;
			if (m_pCounterStrikeSkill->m_skillFrameCount >= m_pCounterStrikeSkill->m_freezeMotionTime)
			{
				StopContinuousTask();
				m_pCounterStrikeSkill->m_skillFrameCount = 0;
				m_pCounterStrikeSkill->m_canImplementFlag = true;
				m_pCounterStrikeSkill->m_hasSpawnGeo = false;
			}
		}
	}*/
}
...
bool AKing::CheckWhetherHaveCSCard()
{
	if (m_allItemCardMap.Contains("CounterStrikeCard")) return true;
	else return false;
}
