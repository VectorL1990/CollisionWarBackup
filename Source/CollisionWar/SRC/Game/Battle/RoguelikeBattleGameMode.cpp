...
void ARoguelikeBattleGameMode::Tick(float DeltaSeconds)
{
	...
	UpdateAllFoundationCharacterIntrinsicEffects();

	//AI模块，只在主机运行.
	UpdateAllCharacterAILogic();
	//UpdateAllFoundationCharacterAILogic();
	UpdateTeamSquads();
	UpdateAINavigation();
	//RCDebug();
...
void ARoguelikeBattleGameMode::UpdateCharacterActionInstruction(const FPlayerOperationInfo& playerOperation, const TArray<FAICharacterOperationInfo>& aiCharacterOperations)
{
	for (int32 i = 0; i < aiCharacterOperations.Num(); i++)
	{
		...
		if (!pCharacter)
			pCharacter = FindFoundationCharacterEntity(aiCharacterOperations[i].entityID);
	...
	if (m_pPlayerCharacter)
	{
		if (playerOperation.actionName == "ImplementSkill" ||
			...
			playerOperation.actionName == "ImplementCounterStrike" ||
			playerOperation.actionName == "ImplementAbsort")
		{
			if (m_pPlayerCharacter->m_pBaseAnimInstance->m_motionStateString != "PMS_NULL" && 
				m_pPlayerCharacter->m_pBaseAnimInstance->m_motionStateString != "PMS_CanCounterStrike")
				m_pPlayerCharacter->m_actionName = "NULL";
			else
			{
				//GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Red, TEXT("BattleGameMode::UpdateCharacterActionInstruction player action now is implementSkill"));
		...
		{
			m_pPlayerCharacter->m_actionName = playerOperation.actionName;
			//m_pPlayerCharacter->m_curEquipNb = m_pPlayerCharacter->m_equipOperationNb;
			//m_pPlayerCharacter->ChangeEquipment(m_pPlayerCharacter->m_curEquipNb);
			//m_pPlayerCharacter->m_curSkillNb = -1;
		}
...
void ARoguelikeBattleGameMode::UpdateAllFoundationCharacterAILogic()
{
	for (uint8 i=0; i<m_pFoundationCharacters.Num(); i++)
	{
		m_pFoundationCharacters[i]->EvaluateShortTask();
		m_pFoundationCharacters[i]->m_hasUpdateAI = true;
	}
}
...
void ARoguelikeBattleGameMode::UpdateAllFoundationCharacterIntrinsicEffects()
{
	for (int32 i=0; i<m_tickIntrinsicInfos.Num(); i++)
	{
		if (!m_tickIntrinsicInfos[i].pHost) continue;
		/*if (m_tickIntrinsicInfos[i].effectName == "ProduceReplenishment")
			UEffectFunLib::ProduceReplenishment(m_tickIntrinsicInfos[i], m_tickIntrinsicInfos[i].pHost);
		else if (m_tickIntrinsicInfos[i].effectName == "ProduceWoodSprite")
			UEffectFunLib::ProduceWoodSprite(m_tickIntrinsicInfos[i], m_tickIntrinsicInfos[i].pHost);
		else if (m_tickIntrinsicInfos[i].effectName == "ProduceThunderStone")
			UEffectFunLib::ProduceThunderStone(m_tickIntrinsicInfos[i], m_tickIntrinsicInfos[i].pHost);
		else if (m_tickIntrinsicInfos[i].effectName == "ProduceFlowLight")
			UEffectFunLib::ProduceFlowLight(m_tickIntrinsicInfos[i], m_tickIntrinsicInfos[i].pHost);*/
	}
}
...
void ARoguelikeBattleGameMode::InitialNewCharacter(ABaseCharacter* pCharacter, int32 campFlag, int32& characterID, FLogicVec2D logicSpawnLoc, bool isAlreadyAssignID)
{
	...
	else if (pCharacter->m_characterType == 2)
	{
		AFoundationCharacter* pFoundationCharacter = Cast<AFoundationCharacter>(pCharacter);
		/*if (pCWGI->m_curPlayerSPInfo.FCLevelInfoList.Contains(pFoundationCharacter->m_characterClassName))
		{
			pFoundationCharacter->NotifyAddLevelPS(pCWGI->m_curPlayerSPInfo.FCLevelInfoList[pFoundationCharacter->m_characterClassName]);
		}*/
		m_pFoundationCharacters.Add(pFoundationCharacter);
		CollectIntrinsicEffectInfos(pFoundationCharacter);
		for (int32 i = 0; i < m_pBPC->m_cardCombineInfo.Num(); i++)
		{
			if (m_pBPC->m_cardCombineInfo[i].triggerFCName != pFoundationCharacter->m_characterClassName) continue;
			if (m_pBPC->m_itemCardInfoMap.Contains(m_pBPC->m_cardCombineInfo[i].combineName))
			{
				UHorizontalBox* pHBox = pFoundationCharacter->m_pCharacterInfoWidget->WidgetTree->ConstructWidget<UHorizontalBox>(UHorizontalBox::StaticClass());
				UTFCInfo* pFCInfoWidget = Cast<UTFCInfo>(pFoundationCharacter->m_pCharacterInfoWidget);
				UPanelSlot* pSlot = pFCInfoWidget->m_pSkillVBox->AddChild(pHBox);
				UVerticalBoxSlot* pVBoxSlot = Cast<UVerticalBoxSlot>(pSlot);
				pVBoxSlot->SetPadding(pFCInfoWidget->m_skillHBoxMargin);
				UTemplateRuneElement* pFCSkill = CreateWidget<UTemplateRuneElement>(m_pBPC, m_pBPC->m_pWidgetManager->m_widgetClassMap["FCSkillInfo"]);
				pFCSkill->NotifyInitial();
				UMaterialInstanceDynamic* pDMI = pFCSkill->m_pRuneImage->GetDynamicMaterial();
				pDMI->SetTextureParameterValue(FName("ButtonTexture"), m_pBPC->m_itemCardInfoMap[m_pBPC->m_cardCombineInfo[i].combineName].pTexture);
				UPanelSlot* pSkillSlot = pHBox->AddChild(pFCSkill);
				UHorizontalBoxSlot* pHBoxSlot = Cast<UHorizontalBoxSlot>(pSkillSlot);
				pHBoxSlot->SetPadding(pFCInfoWidget->m_skillHBoxMargin);
				pFCSkill->m_buttonFunction = "FCSkill_" + m_pBPC->m_cardCombineInfo[i].combineName + "_" + FString::FromInt(pFoundationCharacter->m_entityID);
				for (int32 j=0; j< m_pBPC->m_cardCombineInfo[i].inputList.Num(); j++)
				{
					UTTradeCard* pMaterialCard = CreateWidget<UTTradeCard>(m_pBPC, m_pBPC->m_pWidgetManager->m_widgetClassMap["FCSkillMaterialCard"]);
					pMaterialCard->NotifyInitial();
					if (m_pBPC->m_cardCombineInfo[i].inputList[j].Contains("Color_"))
					{
						FString colorStr = m_pBPC->m_cardCombineInfo[i].inputList[j].RightChop(6);
						uint8 colorInt = 0;
						if (colorStr == "D") colorInt = 0;
						else if (colorStr == "C") colorInt = 1;
						else if (colorStr == "H") colorInt = 2;
						else colorInt = 3;
						
						pMaterialCard->m_pColorImage->Brush.SetResourceObject(m_pBPC->m_pWidgetManager->m_pColorTextureMap[colorInt]);
						pMaterialCard->m_pNbText->SetVisibility(ESlateVisibility::Hidden);
						pMaterialCard->m_pColorImage->SetVisibility(ESlateVisibility::SelfHitTestInvisible);
						
					}
					else if (m_pBPC->m_cardCombineInfo[i].inputList[j].Contains("Number_"))
					{
						FString numberStr = m_pBPC->m_cardCombineInfo[i].inputList[j].RightChop(7);
						pMaterialCard->m_pNbText->SetText(FText::FromString(numberStr));
						pMaterialCard->m_pNbText->SetVisibility(ESlateVisibility::SelfHitTestInvisible);
						pMaterialCard->m_pColorImage->SetVisibility(ESlateVisibility::Hidden);
					}
					UPanelSlot* pMaterialCardSlot = pHBox->AddChild(pMaterialCard);
					UHorizontalBoxSlot* pMatericalCardHBoxSlot = Cast<UHorizontalBoxSlot>(pMaterialCardSlot);
					pMatericalCardHBoxSlot->SetPadding(pFCInfoWidget->m_skillMaterialCardMargin);
				}
			}
		}
	}
...
void ARoguelikeBattleGameMode::CollectIntrinsicEffectInfos(AFoundationCharacter* pCharacter)
{
	for (uint8 i=0; i<pCharacter->m_intrinsicEffectInfo.Num(); i++)
	{
		FIntrinsicEffectInfo intrinsicInfo;
		intrinsicInfo.effectName = pCharacter->m_intrinsicEffectInfo[i].effectName;
		intrinsicInfo.preconditionEffectNames = pCharacter->m_intrinsicEffectInfo[i].preconditionEffectNames;
		intrinsicInfo.effectType = pCharacter->m_intrinsicEffectInfo[i].effectType;
		intrinsicInfo.effectValues = pCharacter->m_intrinsicEffectInfo[i].effectValues;
		intrinsicInfo.pHost = pCharacter;
		if (intrinsicInfo.effectType == 0)
			m_tickIntrinsicInfos.Add(intrinsicInfo);
	}
}

void ARoguelikeBattleGameMode::RemoveIntrinsicEffectInfos(AFoundationCharacter* pCharacter)
{
	int32 i = 0;
	while (i < m_tickIntrinsicInfos.Num())
	{
		if (!m_tickIntrinsicInfos[i].pHost || m_tickIntrinsicInfos[i].pHost == pCharacter)
			m_tickIntrinsicInfos.RemoveAt(i);
		else
			i += 1;
	}
}

bool ARoguelikeBattleGameMode::CheckResourceAmoutToBuild(int32 spellNb)
{
	...
	for (TMap<FString, FFoundationCharacterInfo>::TConstIterator iter = pBPC->m_FCInfos.CreateConstIterator(); iter; ++iter)
	{
		if (spellNb == count)
		{
			for (TMap<int32, int32>::TConstIterator consumptionIter = iter->Value.consumeInfo.hurtLevelConsumptionMap.CreateConstIterator(); consumptionIter; ++consumptionIter)
			{
				if (!m_scoreLevelMaps.Contains(consumptionIter->Key) || m_scoreLevelMaps[consumptionIter->Key] < consumptionIter->Value) return false;
			}
			return true;
		}
		count += 1;
...
void ARoguelikeBattleGameMode::SubtractFCConstructResources(int32 FCNb)
{
	int32 count = 0;
	for (TMap<FString, FFoundationCharacterInfo>::TConstIterator iter = m_pBPC->m_FCInfos.CreateConstIterator(); iter; ++iter)
	{
		if (FCNb == count)
		{
			for (TMap<int32, int32>::TConstIterator consumptionIter = iter->Value.consumeInfo.hurtLevelConsumptionMap.CreateConstIterator(); consumptionIter; ++consumptionIter)
			{
				if (!m_scoreLevelMaps.Contains(consumptionIter->Key)) continue;
				m_scoreLevelMaps[consumptionIter->Key] -= consumptionIter->Value;
			}
			break;
		}
		count += 1;
...
void ARoguelikeBattleGameMode::SpawnConstructionSite(int32 FCNb)
{
	int32 count = 0;
	for (TMap<FString, FFoundationCharacterInfo>::TConstIterator iter = m_pBPC->m_FCInfos.CreateConstIterator(); iter; ++iter)
	{
		if (count == FCNb)
		{
			FFoundationCharacterInfo FCInfo = iter->Value;
			FVector spawnLoc(m_pBPC->m_targetBuildLogicLoc.X, m_pBPC->m_targetBuildLogicLoc.Y, m_defaultSpawnHeight);
			AConstructionSite* pConstructionSite = GetWorld()->SpawnActor<AConstructionSite>(m_constructionSiteClass, spawnLoc, FRotator::ZeroRotator);
			pConstructionSite->m_constructTime = FCInfo.constructionTime;
			pConstructionSite->m_foundationClass = FCInfo.characterClass;
			pConstructionSite->m_occupyInfo = FCInfo.occupyInfo;
			pConstructionSite->m_logicLoc3D = FLogicVec3D(m_pBPC->m_targetBuildLogicLoc.X, m_pBPC->m_targetBuildLogicLoc.Y, m_defaultSpawnHeight);
			pConstructionSite->m_pParticleTemplate = FCInfo.pPS;
			m_pConstructionSites.Add(pConstructionSite);
			break;
		}
		else count += 1;
	}
...
void ARoguelikeBattleGameMode::UpdateConstructionSites()
{
	int32 i = 0;
	while (i < m_pConstructionSites.Num())
	{
		if (!m_pConstructionSites[i]) m_pConstructionSites.RemoveAt(i);
		if (m_pConstructionSites[i]->m_curConstructTime < m_pConstructionSites[i]->m_constructTime)
		{
			m_pConstructionSites[i]->m_curConstructTime += 1;
			i += 1;
		}
		else
		{
			if (m_pConstructionSites[i]->m_foundationClass)
			{
				FVector spawnLoc(m_pConstructionSites[i]->m_logicLoc3D.X, m_pConstructionSites[i]->m_logicLoc3D.Y, m_pConstructionSites[i]->m_logicLoc3D.Z+100);
				AActor* pActor = GetWorld()->SpawnActor<AActor>(m_pConstructionSites[i]->m_foundationClass, spawnLoc, FRotator::ZeroRotator);
				AFoundationCharacter* pFC = Cast<AFoundationCharacter>(pActor);
				int32 id = 1;
				FLogicVec2D logicSpawnLoc(m_pConstructionSites[i]->m_logicLoc3D.X, m_pConstructionSites[i]->m_logicLoc3D.Y);
				InitialNewCharacter(pFC, 0, id, logicSpawnLoc, false);
				pFC->m_occupyInfo = m_pConstructionSites[i]->m_occupyInfo;

				UGameplayStatics::SpawnEmitterAtLocation(pActor, m_pConstructionSites[i]->m_pParticleTemplate, spawnLoc);
			}
			m_pConstructionSites[i]->Destroy();
			if (m_pConstructionSites[i]->IsValidLowLevel())
			{
				m_pConstructionSites[i]->ConditionalBeginDestroy();
			}
			m_pConstructionSites.RemoveAt(i);
		}
	}
}
...
void ARoguelikeBattleGameMode::TriggerAllEventCards()
{
	...
	/*for (int32 i=0; i< m_fullFieldEffectMap.Num(); i++)
	{
	...
	for (TMap<AFoundationCharacter*, FString>::TConstIterator iter = m_allWaitToDealEventCardMap.CreateConstIterator(); iter; ++iter)
	{
		if (!m_pBPC->m_itemCardInfoMap.Contains(iter->Value)) continue;
		m_pPhysCalculator->SpawnEventEffect(m_pBPC->m_itemCardInfoMap[iter->Value].cardEffectParams,
			m_pBPC->m_itemCardInfoMap[iter->Value].cardEffectParams.effectExertType, iter->Value, iter->Key);
	}
	m_allWaitToDealEventCardMap.Empty();
	for (TMap<AFoundationCharacter*, FString>::TConstIterator iter = m_allWaitToSpawnSkillMap.CreateConstIterator(); iter; ++iter)
	{
		for (int32 j = 0; j < iter->Key->m_pBaseEquipment->m_pSkills.Num(); j++)
		{
			if (iter->Key->m_pBaseEquipment->m_pSkills[j]->m_skillName != iter->Value) continue;

			//int32 halfNb = (pSkill->m_spawnGeoNb[i] - pSkill->m_spawnGeoNb[i] % 2) / 2;
			for (int32 k = 0; k < iter->Key->m_pBaseEquipment->m_pSkills[j]->m_mutationGeoClass[-1].geoClasses.Num(); k++)
			{
				if (!iter->Key->m_pBaseEquipment->m_pSkills[j]->m_mutationGeoClass[-1].geoClasses[k]) continue;
				TSubclassOf<class UObject> geoClass = iter->Key->m_pBaseEquipment->m_pSkills[j]->m_mutationGeoClass[-1].geoClasses[k];
				for (int32 n = 0; n < iter->Key->m_pBaseEquipment->m_pSkills[j]->m_spawnGeoNb[k]; n++)
				{
					FVector launchLoc(iter->Key->GetActorLocation().X, iter->Key->GetActorLocation().Y, iter->Key->m_defaultHeight);
					ABasePhysGeo* pGeo = GetWorld()->SpawnActor<ABasePhysGeo>(iter->Key->m_pBaseEquipment->m_pSkills[j]->m_mutationGeoClass[-1].geoClasses[k],
						launchLoc, FRotator::ZeroRotator);
					m_pPhysCalculator->AddPhysGeo(pGeo, iter->Key, iter->Key->m_campFlag);
					pGeo->m_hierachy = 0;
					if (!iter->Key->m_skillParticleMap.Contains(iter->Value)) continue;
					UGameplayStatics::SpawnEmitterAtLocation(pGeo, iter->Key->m_skillParticleMap[iter->Value].particleSystem,
						pGeo->GetActorLocation() + iter->Key->m_skillParticleMap[iter->Value].loc,
						pGeo->GetActorRotation() + iter->Key->m_skillParticleMap[iter->Value].rot);
				}
			}
	...
	m_allWaitToSpawnSkillMap.Empty();
}
