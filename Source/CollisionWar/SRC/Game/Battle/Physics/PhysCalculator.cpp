...
void APhysCalculator::SpawnEventEffect(const FEffectParams& effectParam, uint8 exertType, FString eventName, AFoundationCharacter* pFC)
{
	...
	*/
	...
	for (int32 i=0; i<pExertCharacters.Num(); i++)
	{
		...
		FVector spawnLoc = (pFC->GetActorLocation() + pExertCharacters[i]->GetActorLocation()) / 2.f;
		UParticleSystemComponent* pParticleComponent = NULL;
		pParticleComponent = UGameplayStatics::SpawnEmitterAtLocation(pFC, m_pRGM->m_cardCombinePSMap[eventName].pFCSpawnEffectPS, spawnLoc);
		...
		targetParam.Actor = pExertCharacters[i];
		pParticleComponent->InstanceParameters.Add(soureParam);
		pParticleComponent->InstanceParameters.Add(targetParam);
	}
	/*if (!pFC) return;
	UGameplayStatics::SpawnEmitterAtLocation(this, pBLGM->m_cardCombinePSMap[eventName].pFCSelfSpawnEffectPS,
		pFC->GetActorLocation(), pFC->GetActorRotation(), FVector(1, 1, 1), true);*/
}
