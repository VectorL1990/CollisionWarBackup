...
void UEffectFunLib::Hurt(UEffect* pEffect, ABaseCharacter* pCharacter,const int32& damageCoeK1,const int32& damageCoeK2)
{
	...
	if (pCharacter->m_pBaseAnimInstance && pCharacter->m_pBaseAnimInstance->m_motionStateString == "PMS_Block" && pEffect->m_pHost->m_curSkillNb != -1 &&
		pEffect->m_pHost->m_characterType == 0)
	{
		pCharacter->m_pBaseAnimInstance->m_motionStateString = "PMS_CanCounterStrike";
		AKing* pKing = Cast<AKing>(pCharacter);
		pKing->m_pCSSkill = pEffect->m_pHost->m_pBaseEquipment->m_pSkills[pEffect->m_pHost->m_curSkillNb];
		FString psName = TEXT("CounterStrike");
		pKing->m_pCSAttachPS = pKing->m_skillParticleMap[psName];
		pKing->m_pCSCharacter = pEffect->m_pHost;
		pKing->DyeCSCharacter(pEffect->m_pHost);
	}
	/*else if (pCharacter->m_pBaseAnimInstance && pCharacter->m_pBaseAnimInstance->m_motionStateString == "PMS_Absort" && 
		pEffect->m_pOriginGeo && pCharacter->m_characterType == 1)
	{
		AKing* pKing = Cast<AKing>(pCharacter);
		for (uint8 i=0; i<pEffect->m_pOriginGeo->m_attributeList.Num(); i++)
		{
			if (pKing->m_energyBallList.Num() >= pKing->m_maxEnergyBallNb) break;
			pKing->m_energyBallList.Add(pEffect->m_pOriginGeo->m_attributeList[i]);
			if (pKing->m_pCharacterInfoWidget) pKing->m_pCharacterInfoWidget->UpdateEnergyBall(true, pEffect->m_pOriginGeo->m_attributeList[i]);
		}
	}*/
...
void UEffectFunLib::HurtNoHost(UEffect* pEffect, ABaseCharacter* pCharacter)
{
	...
	/*if (pCharacter->m_pBaseAnimInstance && pCharacter->m_pBaseAnimInstance->m_motionStateString == "PMS_Absort" &&
		pEffect->m_pOriginGeo && pCharacter->m_characterType == 1)
	{
		AKing* pKing = Cast<AKing>(pCharacter);
		for (uint8 i = 0; i < pEffect->m_pOriginGeo->m_attributeList.Num(); i++)
...
void UEffectFunLib::PromotePlayerRecoverRate(FIntrinsicEffectInfo& effectInfo, ABaseCharacter* pLaunchCharacter)
{
	AGameModeBase* pGameMode = UGameplayStatics::GetGameMode(pLaunchCharacter);
	if (!pGameMode) return;
	ABattleLevelGameModeBase* pBattleLevelGameMode = Cast<ABattleLevelGameModeBase>(pGameMode);
	int32 originVal = pBattleLevelGameMode->m_playerRecoverPercent;
	pBattleLevelGameMode->m_playerRecoverPercent += effectInfo.effectValues[0];
	if (pBattleLevelGameMode->m_playerRecoverPercent >= 100) pBattleLevelGameMode->m_playerRecoverPercent = 100;
	effectInfo.modifyEffectValues.Add(pBattleLevelGameMode->m_playerRecoverPercent - originVal);

}

void UEffectFunLib::PromotePlayerRecoverRate_End(FIntrinsicEffectInfo& effectInfo, ABaseCharacter* pLaunchCharacter)
{
	AGameModeBase* pGameMode = UGameplayStatics::GetGameMode(pLaunchCharacter);
	if (!pGameMode) return;
	ABattleLevelGameModeBase* pBattleLevelGameMode = Cast<ABattleLevelGameModeBase>(pGameMode);
	pBattleLevelGameMode->m_playerRecoverPercent -= effectInfo.modifyEffectValues[0];
}
