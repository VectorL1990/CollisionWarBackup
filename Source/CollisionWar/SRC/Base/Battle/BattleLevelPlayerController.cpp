...
void ABattleLevelPlayerController::DealLeftClickDownEvent()
{
	...
	if (!hitSolid)
	{
		if (m_viewTargetState == 1)
		{
			...
			{
				if (pBattleLevelGameMode->m_pFoundationCharacters.Num() < pBattleLevelGameMode->m_maxFCNb)
				{
					FVector cursorLoc(hitResult.Location.X, hitResult.Location.Y, 110.f);
					bool canBuild = CheckWhetherAbleToBuild(cursorLoc, m_curFCOccupyInfo.characterOccupationRadius);
					if (!canBuild) return;
					if (m_isInTutorial && pBattleLevelGameMode->m_tutorialSteps.Num() > 0)
					{
						FString tutorialStepName = "BuildFC";
						if (pBattleLevelGameMode->m_tutorialSteps[0].operationName == tutorialStepName)
							pBattleLevelGameMode->DeleteTutorialStep();
						/*if (pBattleLevelGameMode->m_tutorialSteps[0].operationName != tutorialStepName)
						{
					...
					pBattleLevelGameMode->m_playerOperation.collectPhysID = m_curSelectFCNb;
					m_targetBuildLogicLoc.X = hitResult.Location.X;
					m_targetBuildLogicLoc.Y = hitResult.Location.Y;
				}
			}
		}
		else
	...
	m_curSelectFCNb = -1;
	if (m_pOccupyInfoActor && m_pOccupyInfoActor->IsValidLowLevel())
	{
		m_pOccupyInfoActor->Destroy();
		m_pOccupyInfoActor->ConditionalBeginDestroy();
		m_pOccupyInfoActor = NULL;
	}
}
...
void ABattleLevelPlayerController::DealTickEvent(float dT)
{
	...
	if (!hitResult.bBlockingHit)
	{
		/*if (m_pHoveredCharacter)
		{
			m_pHoveredCharacter->HoveredCancel();
			if (m_pBattleMenu) m_pBattleMenu->NotifyCharacterBriefDisappear();
		}*/
		return;
	}
	if (m_curSelectFCNb != -1 && m_viewTargetState == 1)
	{
		FVector cursorLoc(hitResult.Location.X, hitResult.Location.Y, 110.f);
		if (m_pOccupyInfoActor)
		{
			m_pOccupyInfoActor->SetActorLocation(cursorLoc);
			bool canBuild = CheckWhetherAbleToBuild(cursorLoc, m_curFCOccupyInfo.characterOccupationRadius);
			TArray<UActorComponent*> pStaticMeshComponents = m_pOccupyInfoActor->GetComponentsByClass(UStaticMeshComponent::StaticClass());
			UStaticMeshComponent* pSMComponent = Cast<UStaticMeshComponent>(pStaticMeshComponents[0]);
			if (!canBuild) pSMComponent->CustomDepthStencilValue = 200;
			else pSMComponent->CustomDepthStencilValue = 180;
		}
		else
		{
			m_pOccupyInfoActor = GetWorld()->SpawnActor<AFCDetecter>(m_occupyInfoActorClass, cursorLoc, FRotator::ZeroRotator);
			/*float scale = m_curFCOccupyInfo.characterOccupationRadius / 20.f;
			m_pOccupyInfoActor->SetActorScale3D(FVector(scale, scale, scale));*/

			TArray<UActorComponent*> pStaticMeshComponents = m_pOccupyInfoActor->GetComponentsByClass(UStaticMeshComponent::StaticClass());
			UStaticMeshComponent* pSMComponent = Cast<UStaticMeshComponent>(pStaticMeshComponents[0]);
			bool canBuild = CheckWhetherAbleToBuild(cursorLoc, m_curFCOccupyInfo.characterOccupationRadius);
			if (!canBuild)
				pSMComponent->CustomDepthStencilValue = 200;
			else
				pSMComponent->CustomDepthStencilValue = 180;
			
			//UMaterialInstanceDynamic* pMI = UKismetMaterialLibrary::CreateDynamicMaterialInstance(this, m_pOccupyInfoActorMat);
			//pMI->SetVectorParameterValue(FName(TEXT("BaseColor")), FLinearColor(1, 0, 0, 0.2));
			/*TArray<UActorComponent*> pStaticMeshComponents = m_pOccupyInfoActor->GetComponentsByClass(UStaticMeshComponent::StaticClass());
			UStaticMeshComponent* pSMComponent = Cast<UStaticMeshComponent>(pStaticMeshComponents[0]);
			pSMComponent->SetVectorParameterValueOnMaterials(FName(TEXT("BaseColor")), FVector(1, 0, 0));*/
		}
	}
	/*if (hitResult.GetComponent()->ComponentHasTag(FName(TEXT("BaseCharacterMesh"))))
	{
		ABaseCharacter* pCharacter = Cast<ABaseCharacter>(hitResult.GetActor());
		if (pCharacter)
		{
			...
			m_pHoveredCharacter = pCharacter;
			pCharacter->CursorHovered();
			if (m_pBattleMenu) m_pBattleMenu->NotifyCharacterBriefAppear(pCharacter);
		}
	}
	else
	{
	...
	if (m_viewTargetState == 0)
	{
		...
		if (pBattleLevelGameMode->m_pPlayerCharacter->m_pBaseAnimInstance->m_motionStateString == "PMS_TransientPunch" ||
			pBattleLevelGameMode->m_pPlayerCharacter->m_pBaseAnimInstance->m_motionStateString == "PMS_Absort") return;
...
void ABattleLevelPlayerController::DealRightClickDownEvent()
{
	...
	if (m_viewTargetState == 0)
	{
		...
		if (pBattleLevelGameMode->m_pPlayerCharacter->m_curEquipNb == -1)
		{
			...
			else if (pBattleLevelGameMode->m_pPlayerCharacter->m_pBaseAnimInstance->m_motionStateString == "PMS_CanCounterStrike")
			{
				pBattleLevelGameMode->m_playerOperation.actionName = TEXT("ImplementCounterStrike");
				pBattleLevelGameMode->m_playerOperation.skillNb = -1;
				pBattleLevelGameMode->m_playerOperation.equipmentNb = pBattleLevelGameMode->m_pPlayerCharacter->m_curEquipNb;
				pBattleLevelGameMode->m_playerOperation.skillClickLoc = hitResult.Location;
			}
...
void ABattleLevelPlayerController::ClickBlockKey()
{
	if (m_viewTargetState == 0)
	{
		...
		if (pBattleLevelGameMode->m_pPlayerCharacter->m_pBaseAnimInstance->m_motionStateString == "PMS_TransientPunch" ||
			...
			pBattleLevelGameMode->m_pPlayerCharacter->m_pBaseAnimInstance->m_motionStateString == "PMS_CanCounterStrike" ||
			pBattleLevelGameMode->m_pPlayerCharacter->m_pBaseAnimInstance->m_motionStateString == "PMS_Absort") return;
...
void ABattleLevelPlayerController::ClickAbsortKey()
{
	if (m_viewTargetState == 0)
	{
		AGameModeBase* pGameModeBase = UGameplayStatics::GetGameMode(GetWorld());
		...
		if (!pBattleLevelGameMode->m_pPlayerCharacter) return;
		if (pBattleLevelGameMode->m_pPlayerCharacter->m_pBaseAnimInstance->m_motionStateString == "PMS_TransientPunch" ||
			pBattleLevelGameMode->m_pPlayerCharacter->m_pBaseAnimInstance->m_motionStateString == "PMS_Absort" ||
			pBattleLevelGameMode->m_pPlayerCharacter->m_pBaseAnimInstance->m_motionStateString == "PMS_CanCounterStrike" ||
			pBattleLevelGameMode->m_pPlayerCharacter->m_pBaseAnimInstance->m_motionStateString == "PMS_Block") return;
		//m_isPressBlockKey = true;
		pBattleLevelGameMode->m_playerOperation.actionName = TEXT("ImplementAbsort");
		pBattleLevelGameMode->m_playerOperation.skillNb = -1;
		pBattleLevelGameMode->m_playerOperation.equipmentNb = -1;
	}
}
...
bool ABattleLevelPlayerController::CheckWhetherAbleToBuild(FVector cursorLoc, int32 occupyRadius)
{
	if (!m_pOccupyInfoActor || m_pOccupyInfoActor->m_pOverlapActors.Num() > 0) return false;
	else return true;
	/*if (preconditionNames.Num() == 0) return canBuild;
	canBuild = false;
	FVector affectBoxHalfExtent(affectRadius, affectRadius, 200.f);
	TArray<FOverlapResult> affectOverlapResults;
	GetWorld()->OverlapMultiByChannel(affectOverlapResults, cursorLoc, FQuat::Identity, ECollisionChannel::ECC_Visibility, FCollisionShape::MakeBox(affectBoxHalfExtent));
	for (int32 i=0; i<affectOverlapResults.Num(); i++)
	{
		if (!affectOverlapResults[i].GetActor() || (!affectOverlapResults[i].GetComponent()->ComponentHasTag(FName(TEXT("BasePhysActor"))) &&
			!affectOverlapResults[i].GetComponent()->ComponentHasTag(FName(TEXT("DumpActor"))))) continue;
		if (affectOverlapResults[i].GetComponent()->ComponentHasTag(FName(TEXT("BasePhysActor"))))
		{
			ABasePhysActor* pPhysActor = Cast<ABasePhysActor>(affectOverlapResults[i].GetActor());
			if (!pPhysActor) continue;
			for (uint8 j = 0; j < preconditionNames.Num(); j++)
			{
				if (pPhysActor->m_className.Contains(preconditionNames[j]))
				{
					canBuild = true;
					break;
				}
		...
		else if (affectOverlapResults[i].GetComponent()->ComponentHasTag(FName(TEXT("DumpActor"))))
		{
			ADumpPhysActor* pPhysActor = Cast<ADumpPhysActor>(affectOverlapResults[i].GetActor());
			if (!pPhysActor) continue;
			for (uint8 j = 0; j < preconditionNames.Num(); j++)
			{
				if (pPhysActor->m_className.Contains(preconditionNames[j]))
...
void ABattleLevelPlayerController::LaunchSwitchViewTarget(float deltaT)
{
	//if (m_viewTargetState == 1 || m_viewTargetState == 3) return;
	if (m_viewTargetState == 1)
	{
		if (m_pOccupyInfoActor && m_pOccupyInfoActor->IsValidLowLevel())
		{
		...
		if (pBattleGameMode->m_pPlayerCharacter)
			NotifyPlayPPTimeLine(pBattleGameMode->m_pPlayerCharacter->GetActorLocation(), false);
		if (m_pBattleMenu)
			m_pBattleMenu->SwitchSkillAndBuildDomain(true);
	}
	else
	{
		m_viewTargetState = 1;
		...
		ABattleLevelGameModeBase* pBattleGameMode = Cast<ABattleLevelGameModeBase>(pGameMode);
		if (pBattleGameMode->m_pPlayerCharacter)
			NotifyPlayPPTimeLine(pBattleGameMode->m_pPlayerCharacter->GetActorLocation(), true);
		if (m_pBattleMenu)
			m_pBattleMenu->SwitchSkillAndBuildDomain(false);
	}
}
