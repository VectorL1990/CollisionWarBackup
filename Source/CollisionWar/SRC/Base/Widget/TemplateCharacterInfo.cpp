...
{
	if (!m_pHpBar) return;
	/*FString maxHpStr = FString::FromInt(m_maxHp);
	FString bar = TEXT("/");
	FString hpStr = (maxHpStr.Append(bar)).Append(maxHpStr);
	m_pHpBar->m_pHpText->SetText(FText::FromString(hpStr));
	FString maxDFCStr = FString::FromInt(m_maxDFC);
	FString DFCStr = (maxDFCStr.Append(bar)).Append(maxDFCStr);
	m_pHpBar->m_pDFCText->SetText(FText::FromString(DFCStr));
	FString atkStr = FString::FromInt(m_atkAddition);
